  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      scrollTimer = window.setTimeout(() => {
        pageScrolling = false;
        if (componentVisible && !document.hidden) startRAF();
      }, 120);
    };
    window.addEventListener('scroll', onScroll, { passive: true });
    const onVisibilityChange = () => {
      if (document.hidden) stopRAF();
      else if (componentVisible && !pageScrolling) startRAF();
    };
    document.addEventListener('visibilitychange', onVisibilityChange);

    const render = t => {
      if (pageScrolling || document.hidden) { raf = 0; return; }
    ...
    if (suspendWhenOffscreen) {
      const io = new IntersectionObserver(entries => {
        componentVisible = entries.some(e => e.isIntersecting);
        if (componentVisible && !pageScrolling && !document.hidden) startRAF(); else stopRAF();
      });
